...
// Run the layout effect on the client, no-op on the server (avoids SSR warning
// while still setting the hidden state before paint → no flash of content).
...
export default function Work({
  ...
  useIsoLayoutEffect(() => {
    ...
    // Sync ScrollTrigger with Lenis smooth scroll if it's running.
    const lenis = (
    ...
    const ctx = gsap.context(() => {
      ...
      // Desktop — 3D flip-up reveal + scroll-linked parallax on each glyph.
      mm.add(
        ...
        () => {
          ...
          cards.forEach((cardEl) => {
            const glyph = cardEl.querySelector(".work-glyph");
            if (!glyph) return;
            gsap.fromTo(
              glyph,
              { yPercent: -16 },
      ...
      // Mobile / tablet — the lightweight fade-up (unchanged in feel).
      mm.add(
  ...
  return (
    <section id="work" ref={rootRef} className="border-b border-line bg-paper">
      <div className="mx-auto max-w-6xl px-6 py-20 sm:py-28">
        <div className="flex flex-wrap items-end justify-between gap-4 border-b border-line pb-6">
          ...
          <div className="flex items-center gap-4">
            <span className="hidden font-mono text-xs uppercase tracking-wider text-inkmuted sm:block">
              {projects.length} frames on canvas
            </span>
            <a
              href="#experience"
        ...
        <div
          className="mt-14 grid gap-x-8 gap-y-16 sm:grid-cols-2"
          style={{ perspective: 1200 }}
        >
          {projects.map((project, i) => (
            <div
              key={project.id}
              className={`work-card ${project.featured ? "sm:col-span-2" : ""}`}
            >
